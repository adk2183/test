...
public class MyHashMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    public V get(K key) {
        ...
        if(table[index]==null){
            ...
            while(current!=null){ //this seems right
                if(current.key.equals(key)) {
                    //remove the current and CONNECT
                    //key equals the same key
    ...
    public V put(K key, V value) {
        ...
        int index = key.hashCode()%table.length;
        Entry<K, V > insertingNode = new Entry<>(key,value);
        if(table[index]==null){
            table[index] = insertingNode;
            numEntries++;
            ...
            //save the oldValue
            while(oldValue != null){
                if(oldValue.key.equals(insertingNode.key)){
                    V old = oldValue.value;
                    oldValue.value = insertingNode.value;
                    return old;
                }
                oldValue = oldValue.next;
            }
            //already contains something
            insertingNode.next = table[index];
            table[index] = insertingNode;
            numEntries++;
            if(getLoadFactor() > MAX_LOAD_FACTOR && primeIndex < primes.length - 1){
                rehash();
            }
            //set new entry into index
            return null;
            //CHAIN
        }
        }
    ...
    private void rehash() {
        ...
        for(int i = 0; i<table.length; i++){
            Entry<K, V> current = table[i];
                while(current!=null){
                    Entry<K,V> currentCopy = current;
                    current = current.next;
                    int index = currentCopy.key.hashCode()%newTable.length;

             //       if(newTable[index] != null){
                        oldValue = newTable[index];
                        currentCopy.next = oldValue;
                        newTable[index] = currentCopy;
                 //   }
                 //   else {
                //        newTable[index] = currentCopy;
                //    }
    ...
    public V remove(K key) {
        ...
        int index = key.hashCode()%table.length;
        Entry<K,V> current = table[index];
        Entry<K,V> previous = null; //why is there syntax error
